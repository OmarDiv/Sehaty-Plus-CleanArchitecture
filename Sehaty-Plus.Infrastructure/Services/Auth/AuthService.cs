...
namespace Sehaty_Plus.Infrastructure.Services.Auth
{
        ...
        ) : IAuthService
    {
        ...
        public async Task<Result> RegisterAdminAsync(RegisterUser request, CancellationToken cancellationToken)
        {
            if ((await _userManager.Users.AnyAsync(x => x.Email == request.Email)))
                return Result.Failure(UserErrors.DuplicatedEmail);
            ...
            if (result.Succeeded)
            {
                ...
                _logger.LogInformation("User registered with Code: {code}", code);
